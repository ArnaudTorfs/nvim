...
function myplugin.move_to_error_line()
	...
	local error_line = vim.api.nvim_buf_get_lines(vim.api.nvim_win_get_buf(current_win), current_line_number - 1,
	        current_line_number, false)[1]
...
function myplugin.display_error_count()
	...
	for line_num, line_table in pairs(error_lines) do
		...
		if #line > content_width - 2 then
			truncated_error_lines[line_num] = line:sub(1, content_width - 2) .. "⋯"
		else
	...
	vim.api.nvim_buf_set_lines(content_buf, 0, -1, true, vim.tbl_values(truncated_error_lines))
	...
	vim.api.nvim_command("autocmd WinClosed <buffer> silent! execute 'bwipeout! " ..
	content_buf .. " " .. border_buf .. "'")

	-- Close the floating window when 'q' is pressed
	vim.api.nvim_buf_set_keymap(content_buf, 'n', 'q', '<cmd>close<CR>', { noremap = true, silent = true })

	-- Close the floating window and move to the corresponding line in the original document when Enter is pressed
	vim.api.nvim_buf_set_keymap(content_buf, 'n', '<CR>', ':lua require("myplugin").move_to_error_line()<CR>',
	    { noremap = true, silent = true })
end

function myplugin.setup()
	vim.api.nvim_buf_set_keymap(0, 'n', '<Leader>z', ':lua require("myplugin").display_error_count()<CR>',
	    { noremap = true, silent = true })
end
